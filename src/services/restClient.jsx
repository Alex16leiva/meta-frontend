...
import { waitControlHide, waitControlShow } from "../components/Controls/WaitControl";
    ...
    static httpDelete = (
        ...
        return fetch(`${urlBase}${url}`, {
            ...
            .catch(error => {
                ...
                ToastNotificationService.Error(error.message);
            })
            .then(response => {
    ...
    static httpGet = (
        ...
        return fetch(`${urlBase}${paramUrl}`, options)
            ...
            .then(response => {
                ...
                if (response && response.status && response.status === 404) {
                    ToastNotificationService.Error(response.statusText);
                    return response;
                }
                return isEvaluateMessage ? showValidationMessage(response) : response;
            });
...
const showValidationMessage = (response) => {
    ...
    if (response) {
        if (response.status === 500) {
            ToastNotificationService.Error(JSON.stringify(response));
        }
        if (!utilsValidator.isNullOrEmpty(response.message)) {
            ToastNotificationService.Warn(response.message);
        }
        if (!utilsValidator.isNullOrEmpty(response.ValidationErrorMessage)) {
            ToastNotificationService.Warn(response.ValidationErrorMessage);
        }
        if (!utilsValidator.isNullOrEmpty(response.SuccessMessage)) {
            ToastNotificationService.Success(response.SuccessMessage);
        }
    }
    return response;
}